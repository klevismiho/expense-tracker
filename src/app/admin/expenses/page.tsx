...
const ExpensesPage = () => {
    ...
    const downloadCSV = () => {
        const headers = ["Date", "Category", "Comment", "Amount"];

        const rows = expenses.map((e) => [
            e.date,
            e.category?.name ?? "",
            `"${(e.comment ?? "").replace(/"/g, '""')}"`, // escape quotes in comments
            e.amount,
        ]);
    ...
    return (
        <div className="space-y-8">
            ...
            <button
                onClick={downloadCSV}
                className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-white text-sm rounded-xl hover:bg-slate-700 transition-colors"
            >
                ↓ Download CSV
            </button>
        </div>
